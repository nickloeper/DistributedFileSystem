...
public class BootupUpdate implements Runnable {
	...
	public BootupUpdate(MutableBoolean isPrimary, InetAddress frontendIP, HashMap<String, Integer> fileList, InetAddress rm2IP, InetAddress rm3IP) throws Exception {
	...
	public void run() {
		if(!isPrimary.getValue()) {
			...
			if(primaryId == 3) {
				try {
					getPrimFileList = new UnicastUtilities(ports[primaryId-1], IP3);
					getPrimaryFileList(getPrimFileList);
					getPrimFileList.closeSocket();
				} catch (Exception e) {
					e.printStackTrace();
	...
	private void sendList() {
		ClientDataSerializer mySer = new ClientDataSerializer();
		try {
			UnicastUtilities sendList = new UnicastUtilities(6501);
	...
	private void streamFiles() {
		...
		try {
			receiveNeededFiles = new UnicastUtilities(6501);
	...
	private void sendNeededFilesList() {
		...
		if(primaryId == 3) {
			try {
				sendNeededFiles = new UnicastUtilities(ports[primaryId-1], IP3);
				sendNeededFiles.sendObject(mySer.serializeManagerPacket(neededFiles));
			} catch (Exception e) {
				e.printStackTrace();
			}
