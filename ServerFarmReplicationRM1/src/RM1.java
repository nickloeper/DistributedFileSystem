...
public class RM1 {
	public static void main(String[] args) throws Exception {
		...
		linkData RM2 = new linkData(InetAddress.getByName(args[0]), 9002); 
		...
		UnicastUtilities frontendListener = new UnicastUtilities(6001); 
		
		/*Server information*/
		UnicastUtilities primaryServer = new UnicastUtilities(9101, InetAddress.getByName("localhost")); // TODO Keep as local host
		...
		while(true) {
			while(isPrimary.getValue()) {
				...
				Thread fileOtherRms = new Thread(new SendFileOtherRMs(rmData.get(2), rmData.get(3)));
				fileOtherRms.start();
				fileOtherRms.join();
			}
			while(!isPrimary.getValue()) {
