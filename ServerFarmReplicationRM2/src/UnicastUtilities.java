...
public class UnicastUtilities {
	...
	public void sendFile(byte[] object) throws Exception{
		DatagramPacket sendFile = new DatagramPacket(object, object.length, IP, port);
		socket.send(sendFile);
	}
	
	public byte[] receiveFile(int size) throws Exception {
	...
	public String receiveStringTimeout() throws Exception {
		byte[] receive = new byte[1024];
		DatagramPacket receivePacket=  new DatagramPacket(receive, receive.length);
