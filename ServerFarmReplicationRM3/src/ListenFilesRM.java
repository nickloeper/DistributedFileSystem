...
public class ListenFilesRM implements Runnable {
	...
	private byte[] receiveFile() {
		try {
			receiveFile = new UnicastUtilities(8103);
			...
			receiveFile.closeSocket(); /*Send to server to give to other servers*/
		} catch (Exception e) {
	...
	private void sendToPrimaryServer(byte[] file) {
		try {
			UnicastUtilities sendToPrimary = new UnicastUtilities(4503, InetAddress.getLocalHost());
