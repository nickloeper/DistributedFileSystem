...
public class HandleClientRequest implements Runnable {
	...
	public HandleClientRequest(clientData request) throws Exception {
		...
		server2 = new UnicastUtilities(4302, InetAddress.getLocalHost());
		server3 = new UnicastUtilities(4303, InetAddress.getLocalHost());
	}
	...
	private void updateServer(UnicastUtilities serverCommunicator) {
		try {
			serverCommunicator.sendString("3202");
			sendFile(3202, serverCommunicator);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
